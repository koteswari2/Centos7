...
# Read routines.sh
if ! test -r "/opt/VBoxGuestAdditions-5.2.22/routines.sh"; then
    echo 1>&2 "Required file routines.sh not found.  Aborting..."
    return 1
fi
. "/opt/VBoxGuestAdditions-5.2.22/routines.sh"
...
echo 1>&2 "Removing installed version 5.2.22 of VirtualBox Guest Additions..."
...
for i in "/opt/VBoxGuestAdditions-5.2.22/init/vboxadd-service" "/opt/VBoxGuestAdditions-5.2.22/init/vboxadd"; do
    if test -r "$i"; then
...
for CUR_MODULE in /opt/VBoxGuestAdditions-5.2.22/installer/module-autologon
    do
        . "$CUR_MODULE"
    done
